...
**單一地點（該座標只有一台機器）**
- 不綁 Leaflet popup，點擊 marker 直接：mobile 開 bottom sheet 摘要卡／完整詳情，desktop 在側邊欄顯示完整詳情
- 內容統一由 `buildDetailContentHtml(loc, { compact })` 產生，`compact` 目前兩邊都固定傳 `false`（曾經想拿 `compact:true` 做精簡版摘要卡，後來需求改成「摘要卡跟完整詳情內容一樣，只有高度不同」，這個參數留著但暫時沒在用）
...
- 選了清單項目後 popup 保持開著（兩個平台都是），mobile 開 sheet 顯示摘要卡、desktop 在側邊欄顯示完整詳情
...
- 已知未解問題：手機版 cluster popup 打開後，跟下方新開的 bottom sheet（`z-index:2000`）有機率互相遮蓋，marker 位置太靠畫面下緣時要注意
...
### Mobile Map Bottom Sheet（v20 重寫，跟 v16 版邏輯完全不同）
- Sidebar 從「固定顯示全部地點清單」改為 **marker 驅動**：`sheetLevel` 狀態機為 `'default' | 'summary' | 'full'`，`sheetLoc` 記錄目前顯示的那一筆
- **`default`**（預設，沒點任何 marker）：高度固定 `SHEET_DEFAULT_HEIGHT = 90`，內容是 `.sidebar-placeholder` 提示文字「點選地圖上的圖示，查看機台詳細資訊」，跟桌面版共用同一句文案
- **`summary`**（點了單一地點 marker，或從 cluster popup 清單選了一台）：高度固定 `SHEET_MINI_HEIGHT = 100`，內容跟 `full` 是**同一份** `buildDetailContentHtml(loc, {compact:false})`，差別只在高度——矮的高度靠 `.map-scroll-wrapper` 原生的 `overflow-y:auto` 把超出範圍的部分裁掉，視覺上只露出 badge+標題，不是重新渲染精簡內容
- **`full`**（拖 handle 展開）：高度**動態計算**，抓 `#filterBar`（篩選列）的 `getBoundingClientRect().bottom + 8` 當上限，`window.innerHeight - 上限`，確保不會蓋住搜尋框/篩選器；不再是寫死的 `50vh`
- 拖曳邏輯：`sheetLevelsStack()` 依 `sheetLoc` 是否為 null 回傳 `['default','full']` 或 `['summary','full']`，`initBottomSheet()` 的 touchstart/move/end 一律用這個陣列判斷上下限，沒有「沒選地點時 handle 不能拖」的限制了（原本有，後來拿掉，因為 handle 都顯示出來了理論上就該能拖）
- Cluster marker 不再走 sheet（v20 中途改過一次又改回來，最終版本）：不分裝置一律用浮動 Leaflet popup，見上方「單一地點 vs Cluster」段落
- `closeDetailPanel()`：mobile 收回 `default`（不是完全消失，是矮高度+提示文字），desktop 換回側邊欄提示文字；這是**唯一**能整個關閉/重置的入口，拖曳手勢不會觸發它
- `renderMapLocations()` 資料重新渲染（篩選條件改變）時，開頭就呼叫 `closeDetailPanel()`，避免內容跟新資料對不上；篩選結果為 0 筆時改顯示「找不到符合的地點 இдஇ」蓋掉提示文字
- Mobile 地圖模式的搜尋框/篩選器改成共用 `.toolbar`（跟列表模式同一個元件同一個位置），原本專屬的 `#mapSearchInput`／`.mobile-map-search`／`.map-search-row`／`.mobile-count-badge` 已整個移除（HTML、CSS、JS 都清掉了）
- `map.invalidateSize()` 統一在 `applySheetLevel()` 尾端呼叫（delay 320ms 等 transition 結束）
- Grid view 時側邊欄整個隱藏（`body:not(.map-view) .sidebar { display: none }`），跟 v11 版一致沒變
- `initBottomSheet()` 仍有防重複初始化（`handle.dataset.sheetInit`）

### Desktop 地圖側邊欄（v20 新增，取代 v11 舊版「一直顯示全部清單」）
- 從「一直顯示全部地點清單、sidebar 寬 360px」改為**固定寬 400px 常駐面板**，預設顯示 `.sidebar-placeholder` 提示文字，不是隱藏（`display:none`）——側邊欄本身一直都在，只是內容在「提示文字」跟「完整詳情」之間切換
- 點單一地點 marker → 側邊欄直接顯示完整詳情（一步到位，不像 mobile 分 summary/full 兩階段，因為桌面沒有拖拉手勢）
- 點 cluster marker 的浮動 popup 清單項目 → 側邊欄顯示完整詳情，popup 不會關閉
- 收回：點側邊欄 X，或點地圖空白處（`map.on('click', ...)`，marker 點擊不會冒泡上去，不會誤觸發）
